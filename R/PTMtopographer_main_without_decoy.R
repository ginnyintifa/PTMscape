                                       ...
                                       output_label)
{
  ...
  # 
  # 
  # all_prots_cluster = get_matrix_all(prot_seqs,
  #                                    aaindex_property)
  # 
  # saveRDS(all_prots_cluster,file = paste0(output_label,"_all_prots_cluster_matrix.Rds"))
  # 
  # 
  # all_prots_cluster_df = data.frame(protID = prot_ids, all_prots_cluster, stringsAsFactors = F)
  # 
  # 
  # pos_join_protein_cluster <- left_join(pos,all_prots_cluster_df,by = "protID")%>%
  #   dplyr::select(-c(prot,protID,pos,window,label))
  # pos_join_protein_cluster = mapply(pos_join_protein_cluster,FUN=as.numeric)
  # 
  # candi_join_protein_cluster <- left_join(candi,all_prots_cluster_df,by = "protID")%>%
  #   dplyr::select(-c(prot,protID,pos,window,label))
  # candi_join_protein_cluster = mapply(candi_join_protein_cluster,FUN=as.numeric)
  # 
  # 
  # 
  # 
  # noc_pos_cluster_subtract_mean = noc_pos_cluster-pos_join_protein_cluster
  # noc_candi_cluster_subtract_mean = noc_candi_cluster-candi_join_protein_cluster
  # 
  # 
  # saveRDS(noc_pos_cluster_subtract_mean,file = paste0(output_label,"_noc_pos_cluster_subtract_mean_matrix.Rds"))
  # saveRDS(noc_candi_cluster_subtract_mean,file = paste0(output_label,"_noc_candi_cluster_subtract_mean_matrix.Rds"))
...
### the input is the parameters to retain some of the features and to logit some
### and, the spider properites, and candidate, decoy data from above
### the output is which one is not NA and the written file of not na, no center spider features

spider_feature_extraction = function(candidate_Rds_name,
                                     protID_pos_spider_site_specific,
                                     protID_pos_spider_site_specific_prot_mean,
                                     spider_which_retain,
                                     spider_which_logit,
                                     center_position,
                                     #spider_which_center,
  ...
  
{
  ...
  prot_mean_structure=as.matrix(protID_pos_spider_site_specific_prot_mean[,-1])
  
  prot_mean_structure_extract=extract_site_specific_features(prot_mean_structure,
                                                             spider_which_retain, spider_which_logit, center_position)
  
  rm(prot_mean_structure)
  noc_prot_mean_structure_extract = prot_mean_structure_extract[,-spider_which_center]
  rm(prot_mean_structure_extract)
  
  noc_prot_mean_structure_extract_df = cbind(protID_pos_spider_site_specific_prot_mean[,1],noc_prot_mean_structure_extract)
  
  rm(noc_prot_mean_structure_extract)
  ...
  if(nrow(pos)>0)
  {
    ...
    pos_site_specific = left_join(pos,protID_pos_spider_site_specific,by = c("protID", "pos"))
    # 
    # pos_na_site_specific = apply(as.matrix(pos_site_specific[,-c(1:5)]),1, anyNA)
    # saveRDS(pos_na_site_specific, file = paste0(output_label,"_pos_na_site_specific.Rds"))
    # 
    pos_site_specific_matrix=as.matrix(pos_site_specific[,-c(1:5)])
    rm(pos_site_specific)
    pos_structure=extract_site_specific_features(pos_site_specific_matrix,
                                                 spider_which_retain, spider_which_logit,center_position)
    
    rm(pos_site_specific_matrix)
    
    noc_pos_structure = pos_structure[, -spider_which_center]
    rm(pos_structure)
    
    saveRDS(noc_pos_structure, file = paste0(output_label, "_noc_pos_structure_matrix.Rds"))
    
    pos_mean_structure_df = left_join(pos, noc_prot_mean_structure_extract_df,by = "protID")
    
    
    pos_mean_structure=pos_mean_structure_df[,-c(1:5)]
    
    rm(pos_mean_structure_df)
    
    noc_pos_structure_subtract_mean = noc_pos_structure-pos_mean_structure
    
    saveRDS(noc_pos_structure_subtract_mean, file = paste0(output_label, "_noc_pos_structure_subtract_mean_matrix.Rds"))
    
    rm(noc_pos_structure)
    rm(pos_mean_structure)
    rm(noc_pos_structure_subtract_mean)
  ...
  #### candi
  candi_site_specific = left_join(candi,protID_pos_spider_site_specific,by = c("protID", "pos"))
  # candi_na_site_specific = apply(as.matrix(candi_site_specific[,-c(1:5)]),1, anyNA)
  # saveRDS(candi_na_site_specific, file = paste0(output_label,"_candi_na_site_specific.Rds"))
  
  candi_site_specific_matrix=as.matrix(candi_site_specific[,-c(1:5)])
  rm(candi_site_specific)
  candi_structure=extract_site_specific_features(candi_site_specific_matrix,
                                                 spider_which_retain, spider_which_logit,center_position)
  
  rm(candi_site_specific_matrix)
  
  noc_candi_structure = candi_structure[, -spider_which_center]
  rm(candi_structure)
  
  saveRDS(noc_candi_structure, file = paste0(output_label, "_noc_candi_structure_matrix.Rds"))
  
  candi_mean_structure_df = left_join(candi, noc_prot_mean_structure_extract_df,by = "protID")
  
  
  candi_mean_structure=candi_mean_structure_df[,-c(1:5)]
  
  rm(candi_mean_structure_df)
  
  noc_candi_structure_subtract_mean = noc_candi_structure-candi_mean_structure
  
  saveRDS(noc_candi_structure_subtract_mean, file = paste0(output_label, "_noc_candi_structure_subtract_mean_matrix.Rds"))
  
  rm(noc_candi_structure)
  rm(candi_mean_structure)
  
  rm(noc_candi_structure_subtract_mean)
...
### a function to operate on the whole spider feature matrix 

###  I only need to do this once in the whole prediction process
...
{
# 
#   candidate_Rds_name = "methy_acety_cross_sel.Rds"
#   extracted_spider_Rds_name = "extracted_spider.Rds"
# 
# 
#   spider_which_retain = c(1,6,8,9)
#   spider_which_logit = c(8,9)
#   center_position = 13
# 
...
knn_cleaning = function(pos_matrix_Rds_name, neg_matrix_Rds_name, k, 
                        output_label)
{
  #  pos_matrix = cbind(rnorm(40000),rnorm(40000))
  #  candi_matrix = cbind(rnorm(600000,mean = 2), rnorm(600000, mean =2))
  #  k = 50
  # # 
  # # 
  ...
  cat("ori pos size ",nrow(pos_matrix),"\n")
  cat("ori neg size ",nrow(neg_matrix),"\n")
  
  
  #k = nrow(candi_matrix)/nrow(pos_matrix)
  ### perform z transform on the data 
  searchspace = rbind(pos_matrix, neg_matrix)
  z_searchspace = get_z_transform(searchspace)
  
  
  pos_ind = c(1:nrow(pos_matrix))
  neg_ind = c((nrow(pos_matrix)+1): (nrow(pos_matrix)+nrow(neg_matrix)))
  
  
  
  z_neg_matrix = z_searchspace[neg_ind,]
  
  
  #### a function to calculate distance between one point to a set of points
  #### only keep the min k nearest index
  
  bigs = nrow(z_searchspace)
  
  neg_to_keep = lapply(1:nrow(z_neg_matrix), function(x){
    get_k_keep(z_neg_matrix[x,], z_searchspace, k,pos_ind,bigs)
  })
  ...
  cat("to get knn","\n")
  # ptm = proc.time()
  # getknn = get.knnx(z_searchspace,z_neg_matrix, k)
  # cat(proc.time()-ptm,"\n")
  # 
  # 
  # # count for each candidate site how many index belongs to positive set
  # 
  # index = getknn$nn.index
  # distance = getknn$nn.dist
  # cat("get count of pos neighbor for each neg","\n")
  # #ptm = proc.time()
  # 
  ...
  # 
  # ptm = proc.time()
  # count_of_pos_in_neighbor=vapply(1:nrow(index),function(x) sum(index[x,]%in%pos_ind),0)
  # cat(proc.time()-ptm,"\n")
  # #hist(count_of_pos_in_neighbor)
  # 
  # 
  cat("get clean neg","\n")
  which_clean = which(count_of_pos_in_neighbor==0)
  
  
  saveRDS(neg_to_keep, file = paste0(output_label, "_clean_neg_index.Rds"))
  
  # plot(z_candi_matrix)
  # lines(z_searchspace[pos_ind,], type = "p", col = "red")
  # lines(z_candi_matrix[-which_clean, ], col = "green", type ="p")
  # 
  clean_neg = neg_matrix[neg_to_keep,]
  
  cat("after pos size ",nrow(pos_matrix),"\n")
  cat("after neg size ",nrow(clean_neg),"\n")
  
  saveRDS(clean_neg, file = paste0(output_label, "_clean_neg_feature.Rds"))
  
}



mean_dist_cleaning = function(pos_matrix_Rds_name, neg_matrix_Rds_name, 
                              output_label)
{
  ...
  z_pos_matrix = z_searchspace[pos_ind,]
  z_neg_matrix = z_searchspace[neg_ind,]
  
  ### calculate the mean of the positive ones
  
  pos_mean = apply(z_pos_matrix,2,mean)
  dist_to_pos_mean = rowSums(abs(matrix(pos_mean-as.vector(t(z_neg_matrix)),
                                        nrow = nrow(z_neg_matrix), byrow = T )))
  
  
  #### get the maximum ones
  
  k = nrow(pos_matrix)
  
  p = length(dist_to_pos_mean)-k
  
  xp = sort(dist_to_pos_mean, partial= p)[p]
  
  neg_to_keep = which(dist_to_pos_mean>xp)[1:k]
...
scale_train_test_esa = function(feature_train_name, feature_test_name, 
                                upper_bound, lower_bound,
                                output_label_fortrain, output_label_fortest)
{
...
libsvm_formating_group = function(feature_name, n_fold, output_label)
{
  
  for(i in 1:n_fold)
  {
    feature_matrix_train_Rds_name = paste0(feature_name, "_train_feature_",i, "_scale.Rds")
    feature_label_train_Rds_name = paste0(feature_name, "_train_label_",i, ".Rds")
    
    feature_train_out_Rds_name = paste0(output_label, "_train_feature_",i)
    feature_train_out_tsv_name = paste0(output_label, "_train_feature_",i)
    
    
    feature_matrix_test_Rds_name = paste0(feature_name, "_test_feature_",i, "_scale.Rds")
    feature_label_test_Rds_name = paste0(feature_name, "_test_label_",i, ".Rds")
    
    feature_test_out_Rds_name = paste0(output_label, "_test_feature_",i)
    feature_test_out_tsv_name = paste0(output_label, "_test_feature_",i)
    
    
    
    libsvm_formating(feature_matrix_train_Rds_name, feature_label_train_Rds_name,
                     feature_train_out_Rds_name, feature_train_out_tsv_name)
    
    
    libsvm_formating(feature_matrix_test_Rds_name, feature_label_test_Rds_name,
...
libsvm_formating_train_single = function(train_feature_name,
                                   train_label_name,
                                   output_label)
{
...
plot_pca_for_two_types = function(pos_feature_Rds_name,
  ...
  
{
  ...
  get_pca_plot_two(sel_feature, type_label, paste0(output_label,"_pca_plot_two.pdf"))
...
### for tsne limit each type of sample to 50000

plot_tsne_for_two_types = function(pos_feature_Rds_name,
                                   candi_feature_Rds_name,
                                   output_label)
  
{
  ...
  get_tsne_pos_candi(sel_feature, type_label, paste0(output_label,"_tsne_plot_two.pdf"))
...
get_auc_group = function(prediction_path, label_path,prediction_name, label_name, n_fold,output_fig_name)
{
  
  # 
  # 
  # prediction_path = "/data/ginny/liblinear-2.11/cv_1030/"
  #            label_path =    "cv_1030/"
  #           prediction_name = "py_psvm_size25_nms_balanced10_test_feature"
  #            label_name =    "py_psvm_size25_nms_balanced10_test_label"
  #              n_fold =  10
  #               output_fig_name = "py_psvm_size25_nms_balanced10"
  # 
  # 
  # 
  
  
  record_auc = rep(0,n_fold)
  record_mcc = rep(0,n_fold)
  record_fmeasure = rep(0,n_fold)
  record_sens = rep(0,n_fold)
  record_spec = rep(0,n_fold)
  record_sensf = rep(0,n_fold)
  record_specf = rep(0,n_fold)
  ...
  for(i in 1:n_fold)
  {
    ...
    predict_tsv = paste0(prediction_path, prediction_name, "_",i,"_svm_predict.tsv")
    label_tsv = paste0(label_path, label_name, "_",i, ".tsv")
    
    output_name = paste0(output_fig_name, "_hist_roc_",i,".pdf")
    
    test_predict = data.table::fread(predict_tsv,stringsAsFactors = F, header = T)
    test_label = data.table::fread(label_tsv, stringsAsFactors = F)
    
    
    total_score = c(total_score, test_predict$'1')
    total_label = c(total_label, test_label$V1)
    
    
    roc_test = roc(test_label$V1, test_predict$'1')
    
    pred = prediction(test_predict$'1', test_label$V1)
    
    perf = performance(pred, "mat")
    
    mi = which.max(perf@y.values[[1]])
    cutoff_mi = perf@x.values[[1]][mi]
    t = table(test_predict$'1'>cutoff_mi, test_label$V1)
    ...
    mif = which.max(perff@y.values[[1]])
    cutoff_mif = perff@x.values[[1]][mif]
    
    tf = table(test_predict$'1'>cutoff_mif, test_label$V1)
    
    sensf = tf[2,2]/(tf[2,2]+tf[1,2])
    specf = tf[1,1]/(tf[1,1]+tf[2,1])
    
    
    
    
    
    # cat(roc_test$auc,"\t",max(perf@y.values[[1]], na.rm = T))
    
    # cat(max(perf@y.values[[1]], na.rm = T))
    # cat("\n")
    # 
    # cat("threshold: ", cutoff_mi,"\n")
    # 
    # cat("sens: ", sens, " spec: ",spec)
    # 
    # cat("\n")
    # 
    # 
    # 
    # 
    # cat(max(perff@y.values[[1]], na.rm = T))
    # cat("\n")
    # 
    # cat("threshold_fmeasure: ", cutoff_mif,"\n")
    # 
    # cat("sensf: ", sensf, " specf: ",specf)
    # 
    # cat("\n")
    # 
    ...
    record_auc[i] = roc_test$auc
    
    record_mcc[i] =max(perf@y.values[[1]], na.rm = T)
    
    record_sens[i] = sens
    
    record_spec[i] = spec
    record_fmeasure[i] =max(perff@y.values[[1]], na.rm = T)
    
    record_sensf[i] = sensf
    
    record_specf[i] = specf
    
    
    
    pdf(output_name, useDingbats = F)
    hist(test_predict$'1', breaks = 50) ## see the score distribution
    plot(roc_test)
    dev.off()
  }
  
  cat("mean auc: ", mean(record_auc))
  cat("\n")
  cat("sd auc: ", sd(record_auc))
  cat("\n")
  
  cat("mean mcc: ", mean(record_mcc))
  cat("\n")
  
  
  cat("mean sens: ", mean(record_sens))
  cat("\n")
  
  cat("mean spec: ", mean(record_spec))
  cat("\n")
  
  
  
  cat("mean fmeasure: ", mean(record_fmeasure))
  cat("\n")
  
  
  cat("mean sensf: ", mean(record_sensf))
  cat("\n")
  
  cat("mean specf: ", mean(record_specf))
  cat("\n")
  
  
  
  roc_total = roc(total_label, total_score)
  output_name_total = paste0(output_fig_name, "_hist_roc_total.pdf")
  
  
  pdf(output_name_total, useDingbats = F)
  hist(total_score, breaks = 50) ## see the score distribution
  plot(roc_total)
  dev.off()
  # 
...
get_prediction_MCC = function(first_test_predict_name,
                              second_test_predict_name,
                              first_test_label_name,
                              second_test_label_name,
                              output_label,
                              output_fig_label)
{
  
  first_test_predict =  data.table::fread(first_test_predict_name, stringsAsFactor  = F, header = T)
  second_test_predict =  data.table::fread(second_test_predict_name, stringsAsFactor  = F,header = T)
  
  first_test_label =  data.table::fread(first_test_label_name, stringsAsFactor  = F, header  = F)
  second_test_label =  data.table::fread(second_test_label_name, stringsAsFactor  = F, header = F)
  
  
  #### combine all the positive together and all the candidate together
  
  first_pred_df = cbind(first_test_predict, first_test_label)
  second_pred_df = cbind(second_test_predict, second_test_label)
  
  all_pred_df = rbind(second_pred_df, first_pred_df)
  rm(first_pred_df)
  rm(second_pred_df)
  colnames(all_pred_df) = c("pred_label","pos_score","neg_score","true_label")
  pos_pred_score = all_pred_df%>%dplyr::filter(true_label==1) %>% dplyr::select(pos_score)
  
  candi_pred_score = all_pred_df%>%dplyr::filter(true_label==0)%>% dplyr::select(pos_score)
  positive_score = pos_pred_score$pos_score
  candi_score = candi_pred_score$pos_score

  cat("number of PSP sites: ", length(positive_score), "\n")
  cat("number of candidate sites: ", length(candi_score), "\n")
  saveRDS(positive_score, paste0(output_label, "_positive_score.Rds"))
  saveRDS(candi_score, paste0(output_label, "_candi_score.Rds"))
  
  total_score = c(positive_score,candi_score)
  total_label = c(rep(1,length(positive_score)), rep(0, length(candi_score)))
  ...
  get_8 = abs(record_spec-0.8)
  score_which_8 = which.min(get_8)/1000

  get_9 = abs(record_spec-0.9)
  score_which_9 = which.min(get_9)/1000

  get_95 = abs(record_spec-0.95)
  score_which_95 = which.min(get_95)/1000

  get_99 = abs(record_spec-0.99)
  score_which_99 = which.min(get_99)/1000

  cat("at spec 90%: score,how_many, spec ", score_which_9,length(which(candi_score>score_which_9)) ,record_spec[score_which_9*1000],"\n")
  cat("at spec 95%: score,how_many, spec ", score_which_95,length(which(candi_score>score_which_95)) ,record_spec[score_which_95*1000],"\n")
  cat("at spec 99%: score,how_many, spec ", score_which_99,length(which(candi_score>score_which_99)) ,record_spec[score_which_99*1000],"\n")
  ...
  pdf(paste0(output_fig_label,"_roc.pdf"), useDingbats = F)
  plot(roc_test)
  dev.off()

  pdf(paste0(output_fig_label,"_candidate_score_hist.pdf"), useDingbats = F)
  hist(candi_score,breaks = 50, main = "candidate_score")
  dev.off()

  pdf(paste0(output_fig_label,"_positive_score_hist.pdf"), useDingbats = F)
  hist(positive_score,breaks = 50, main = "candidate_score")
  dev.off()

  pdf(paste0(output_fig_label,"_both_score_dens.pdf"), useDingbats = F)

  pd = density(positive_score)
  cd = density(candi_score)
...
process_n_fold_cv = function(n, process_file_path, model_path, prediction_path,
                             train_feature, test_feature,  cv_log_path_name)
  
{
  
  system("cd /data/ginny/liblinear-2.11/")
  
  sink(cv_log_path_name, append = T)
  ...
  for (i in 1:n)
  {
    ...
    train_feature_name = paste0(process_file_path,train_feature,"_",i,"_svm.tsv")
    test_feature_name = paste0(process_file_path,test_feature,"_",i,"_svm.tsv")

    model_name = paste0(model_path,train_feature,"_",i,"_svm_model_c.tsv")
    test_prediction_name = paste0(prediction_path, test_feature,"_",i,"_svm_predict.tsv")

    
    
    cv_command = paste("./train -s 2 -C", train_feature_name,">> cv_log.txt", sep = " ")
    system(cv_command)
    ...
    training_command = paste("./train -s 2 -c", get_c,train_feature_name,model_name,
                             sep = " ")
    system(training_command)
    
    
    predict_test_command = paste("./predict -b 1",test_feature_name, model_name,
                                 test_prediction_name,">> predict_test_log.txt",
                                 sep = " ")
    system(predict_test_command)
    tit = readLines("predict_test_log.txt")
...
window_score_assembly = function(candidate_df_Rds,
                                 first_pos_train_ind_Rds,
                                 first_pos_test_ind_Rds,
                                 first_candi_train_ind_Rds,
                                 first_candi_test_ind_Rds,
                                 positive_score_Rds,
                                 candi_score_Rds,
                                 output_label)
  
{
  ...
  # candidate_df_Rds = "ps_pred_candidate.Rds"
  # decoy_df_Rds = "ps_pred_decoy.Rds"
  # first_pos_train_ind_Rds = "ps_pred_2_train_pos_ind_1_.Rds"
  # first_pos_test_ind_Rds = "ps_pred_2_test_pos_ind_1_.Rds"
  # first_candi_train_ind_Rds = "ps_pred_2_train_candi_ind_1_.Rds"
  # first_candi_test_ind_Rds = "ps_pred_2_test_candi_ind_1_.Rds"
  # positive_score_Rds = "ps_der_positive_score.Rds"
  # candi_score_Rds = "ps_der_candi_score.Rds"
  # decoy_score_Rds = "ps_der_decoy_score.Rds"
  # output_label = "ps_pred"
  # 
  ...
  first_pos_train_ind = readRDS(first_pos_train_ind_Rds)
  first_pos_test_ind = readRDS(first_pos_test_ind_Rds)
  
  first_candi_train_ind = readRDS(first_candi_train_ind_Rds)
  first_candi_test_ind = readRDS(first_candi_test_ind_Rds)
  ...
  # for pos
  pos_train_df = pos_df[first_pos_train_ind,]
  pos_test_df = pos_df[first_pos_test_ind,]
  
  rm(pos_df)
  
  pos_combine_df = rbind(pos_train_df, pos_test_df)
  
  rm(pos_train_df)
  rm(pos_test_df)

  ### a bug , positive_score and negative socre are first/second are in reverse order 
  
  
  pos_score_df <- pos_combine_df%>%
    dplyr::mutate(prediction_score = positive_score)%>%
    dplyr::arrange(protID, pos)%>%
    dplyr::select(-prot)
  
  
  saveRDS(pos_score_df, paste0(output_label, "_pos_window_score.Rds"))
  write.table(pos_score_df, paste0(output_label, "_pos_window_score.tsv"), sep = "\t",
              row.names = F, quote = F)
  ...
  candi_train_df = candi_df[first_candi_train_ind,]
  candi_test_df = candi_df[first_candi_test_ind,]
  
  rm(candi_df)
  
  candi_combine_df = rbind(candi_train_df, candi_test_df)
  
  rm(candi_train_df)
  rm(candi_test_df)
  
  candi_score_df <- candi_combine_df%>%
    dplyr::mutate(prediction_score = candi_score)%>%
    dplyr::arrange(protID, pos)%>%
    dplyr::select(-prot)
  
  
  saveRDS(candi_score_df, paste0(output_label, "_candi_window_score.Rds"))
  write.table(candi_score_df, paste0(output_label, "_candi_window_score.tsv"), sep = "\t",
              row.names = F, quote = F)
...
create_ggseqlogo_plot = function(candidate_df_Rds, 
                              output_label)
{
  ...
  ggseqlogo(pos_windows$window, seq_type = 'aa') 
...
create_ggseqlogo_plot_for_meta = function(meta_df_Rds, 
                                 output_label)
{
  ...
  colnames(meta_df) = c("protID","pos","window","domain","subcellular","prediction_score",
                       "label")
  
  pos_windows = meta_df %>% 
    dplyr::filter(label == "PSP_pred" | label == "PSP_unpred" | label =="candidate_pred") %>%
    dplyr::select(window)
  delete_center = paste0(substr(pos_windows$window,1,12),substr(pos_windows$window,14,25))
  ...
  ggsave(filename = paste0(output_label,"_meta_logoPlot.pdf"),device = "pdf",
         width = 10, height = 8)
...
create_ggseqlogo_prob_plot = function(candidate_df_Rds, 
                                 output_label)
{
  ...
  ggseqlogo(pos_windows$window, seq_type = 'aa', method = "probability")
  
  #dev.off()
  
  ggsave(filename = paste0(output_label,"_prob_logoPlot.pdf"),device = "pdf",
         width = 10, height = 8)
